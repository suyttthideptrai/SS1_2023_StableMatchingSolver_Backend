...
@Slf4j
@Data
public class StableMatchingProblemExtra implements Problem {
    ...
    private ArrayList<PreferenceListExtra> preferenceLists; // Preference List of each Individual
    private MatchesExtra matches;

    private int numberOfSets;


    private Map<Integer, Boolean> fnStatusEachSet = new HashMap<>();
    ...
    private void initializeFields() {
        ...
        preferenceLists = (ArrayList<PreferenceListExtra>) getPreferences();
    }
    ...
    public StableMatchingProblemExtra(int capacity) {
        this.matches = new MatchesExtra(capacity);
    }

    private boolean isValidEvaluateFunction(String function) {
        return StableMatchingProblemExtra.VALID_EVALUATE_FUNCTION_KEYWORDS
                .stream()
    ...
    public void evaluate(Solution solution) {
        ...
        double fitnessScore;
        fnStatus = fnStatusEachSet.values().stream().allMatch(status -> !status);
        if (!this.fnStatus) {
     ...
     */
    public PreferenceListExtra getPreferenceOfIndividual(int index){
        PreferenceListExtra a ;
        boolean allFalse = fnStatusEachSet.values().stream().allMatch(status -> !status);
    ...
    private List<PreferenceListExtra> getPreferences() {
        List<PreferenceListExtra> fullList = new ArrayList<>();
        for (int i = 0; i < individuals.getNumberOfIndividual(); i++) {
            PreferenceListExtra a = getPreferenceOfIndividual(i);
            fullList.add(a);
        }
        return fullList;
    }
...
//			int padding = individuals.getPaddingOf(Node);
    ...
    private boolean isPreferredOver(int newNode, int currentNode, int SelectorNode) {
        PreferenceListExtra preferenceOfSelectorNode = preferenceLists.get(SelectorNode);
    ...
    private int getLastChoiceOf(int target) {
        PreferenceListExtra pref = preferenceLists.get(target);
    ...
    private int getLeastScoreNode(int selectorNode, int newNode, Integer[] occupiedNodes) {
        PreferenceListExtra prefOfSelectorNode = preferenceLists.get(selectorNode);
    ...
    public double[] getAllSatisfactions(Matches matches) {
        ...
        for (int i = 0; i < numSet0; i++) {
            double setScore = 0.0;
            PreferenceListExtra ofInd = preferenceLists.get(i);
